...
describe('EventForm — mapowanie', () => {
  ...
  it('kilka drużyn: chipy („A + B") ↔ `teams[]` w obie strony', () => {
    const f = { ...emptyForm('turniej', '2026-10-10', '2027-06-30'), team: 'A + B', name: 'T' };
    expect(toRaw(f)).toMatchObject({ teams: ['A', 'B'] });
    expect(toRaw({ ...f, team: '' })).toMatchObject({ teams: [] });
    const e = { id: 'x', type: 'turniej', teams: ['A', 'B'], title: 'T', startsAt: '2026-10-10T06:00:00.000Z', endsAt: '2026-10-10T13:00:00.000Z', allDay: false, place: null, coaches: [], details: {}, seriesId: null, createdBy: 'a', updatedBy: 'a', createdAt: 'c', updatedAt: 'u', deletedAt: null, deletedBy: null } satisfies CalEvent;
    expect(fromEvent(e, '2027-06-30').team).toBe('A + B');
  });
