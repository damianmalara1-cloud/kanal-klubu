...
export function ScopeChoice({ onPick, onCancel, verb }: { onPick: (s: Scope) => void; onCancel: () => void; verb: 'zapisać' | 'usunąć' }) {
  return (
    <div className="stack">
      <h2>Co {verb}?</h2>
      {verb === 'zapisać' && <p className="muted">Data zostaje w każdym terminie, zmieniają się godziny, miejsce, trenerzy.</p>}
      <button className="btn btn-primary" type="button" onClick={() => onPick('one')}>
        Tylko ten termin
      </button>
      <button className="btn" type="button" onClick={() => onPick('following')}>
