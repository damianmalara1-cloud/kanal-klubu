...
const np = (over: Partial<NewPost> = {}): NewPost => ({
  author: 'Ania', type: 'mecz', ip: '1.1.1.1', partnerInfo: true, purgeAfter: '2030-01-01T00:00:00.000Z',
...
describe('MemoryRepo', () => {
  ...
  it('create/get/update/delete', async () => {
    ...
    const u = await repo.update(p.id, { status: 'done', caption: 'x' });
    expect(u.status).toBe('done'); expect(u.updatedAt >= p.updatedAt).toBe(true);
    await repo.delete(p.id);
    expect(await repo.get(p.id)).toBeNull();
  });
  it('listByAuthor sortuje malejąco i limituje', async () => {
    const p1 = await repo.create(np()); await repo.update(p1.id, { status: 'done', createdAt: '2026-01-01T00:00:00.000Z' });
    const p2 = await repo.create(np()); await repo.update(p2.id, { status: 'done', createdAt: '2026-01-02T00:00:00.000Z' });
  ...
  it('listByAuthor sortuje stabilnie po id gdy createdAt identyczny (remis)', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const p = await repo.create(np());
      await repo.update(p.id, { status: 'done', createdAt: tie });
      posts.push(p);
    }
  ...
  it('listByAuthor pomija posty w statusie draft', async () => {
    ...
    await repo.update(a.id, { status: 'done' });
    const b = await repo.create(np());
    await repo.update(b.id, { status: 'done' });
    const l = await repo.listByAuthor('Ania', 10);
  ...
  it('listForPurge', async () => {
    ...
    await repo.update(a.id, { status: 'done' });
    const b = await repo.create(np());
    await repo.update(b.id, { status: 'done', updatedAt: '2000-01-01T00:00:00.000Z' });
  ...
  it('update zwraca kopię — mutacja zwróconego obiektu nie wpływa na store', async () => {
    const p = await repo.create(np());
    const u = await repo.update(p.id, { status: 'done' });
